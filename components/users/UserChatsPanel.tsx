...
export function UserChatsPanel({ userId, enabled = true, className }: UserChatsPanelProps) {
  ...
  }, [enabled, sessionList, selectedSessionId])
  ...
  const sessionHeader = useMemo(() => {
    ...
    if (current.title && current.title.trim()) return current.title
  ...
  return (
    <div className={cn("flex min-h-0 flex-1 flex-col gap-4 md:flex-row", className)}>
      <div className="flex w-full flex-col gap-3 md:w-56 lg:w-72">
      ...
      <div className="flex min-h-0 flex-1 flex-col gap-3">
        <div className="flex items-center justify-between">
          <span className="text-sm font-medium text-foreground truncate">{sessionHeader}</span>
          {selectedSessionId && (
            <Badge variant="secondary" className="text-[10px]">
